 ...
 * <p>
 * None of those synthesizers reports what it is sounding, but every message of the song goes past
 * {@link MldDriver} on its way to one, so what is shown is what it sent - see {@link MldChannels}.
 * A midi note is already a note, so these keys are exact.
 * <p>
 * The rows are the chip's: FM for a Yamaha song, PCM for a FueTrek or a Rohm one. Sixteen channels
 * do not fit nine rows, so they are taken in the order they first sound, the way {@link MidiReader}
 * does. The analyzer bars are not this reader's business: the song is rendered through mdplayer's
 * own mixer, so they are measured off the sound itself.
 *
 * @author <a href="mailto:umjammer@gmail.com">Naohide Sano</a> (nsano)
 * @version 0.00 2026-09-19 nsano initial version <br>
 */
public class MldReader implements FmDspChipReader {

    private static final int CHANNELS = MldChannels.CHANNELS;
    ...
    public boolean ready() {
        return channels() != null && chip() != null;
    }

    @Override
    public void poll() {
        MldChannels channels = channels();
